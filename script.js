...
function Column(name) { 
	...
	function createColumn() {
		...
		var $colheader = $('<div>').addClass('columnheader');
		var $columnTitle = $('<h2>').addClass('column-title').text(self.name);
		var $columnCardList = $('<ul>').addClass('column-card-list');
		var $columnDelete = $('<button>').addClass('btn-delete').text('×');
		var $columnAddCard = $('<button>').addClass('btn-add-card').text('Add card');
...
function initSortable() {
	$('.column-card-list').sortable({
		connectWith: '.column-card-list', placeholder: 'card-placeholder'
	})
	.disableSelection();
}
